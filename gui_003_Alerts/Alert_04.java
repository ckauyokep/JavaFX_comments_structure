package gui_003_Alerts;

import javafx.application.Application;
import javafx.stage.Modality;
import javafx.stage.Stage;
import javafx.scene.Scene;
import javafx.scene.layout.StackPane;
import javafx.scene.layout.VBox;
import javafx.scene.control.Button;
import javafx.scene.control.Label;
import javafx.event.ActionEvent;
import javafx.event.EventHandler;
import javafx.geometry.Pos;

public class Alert_04 extends Application {

Stage window;
Scene scene1, scene2;
Button button;

public static void main(String[] args) {
	launch(args);
}

	@Override
	public void start(Stage primaryStage) {
		
		// 3. Создаём окно
		window = primaryStage;
		// 3.1 Добавляем к окну реакцию при закрытии
		window.setOnCloseRequest(e -> {
			e.consume();
			closeProgram();
		});
		
		// 1. Создаём элемент
		button = new Button("Выйти");
		// 1.1 Добавляем к кнопке реакцию при нажатии
		button.setOnAction(e -> {
			ConfirmBox confirmbox = new ConfirmBox("Выйти", "Вы уверены, что хотите закрыть приложение ?");
			boolean answer = confirmbox.display();
			System.out.println(answer);
			if(answer) {
				window.close();
			}
		});
		
		// 2. Создаём вёрстку элементов в окне
		StackPane layout = new StackPane();
		// 2.1 Добавляем элементы в окно
		layout.getChildren().add(button);
		
		// 3.2 Создаём титульное название окна
		window.setTitle("Sync My Data");
		// 3.3 Создаём размеры окна
		Scene scene = new Scene(layout, 800, 600);
		// 3.4 Создаём внутреннее пространство
		window.setScene(scene);
		// 3.5 Показываем окно
		window.show();
		
	}
	
	// 3.1.1 Создаём для окна реакцию при закрытии
	void closeProgram() {
		ConfirmBox confirmbox = new ConfirmBox();
		boolean answer = confirmbox.display();
		System.out.println(answer);
		if(answer) {
			window.close();
		}
	}
	
	class ConfirmBox {
		boolean answer;
		String title;
		String message;
		
		// Создаём конструктор
		ConfirmBox() {
			this.title = "Выйти";
			this.message = "Вы уверены, что хотите закрыть приложение ?";
		}
		
		// Создаём конструктор
		ConfirmBox(String title, String message) {
			this.title = title;
			this.message = message;
		}
		
		boolean display() {
			// 1. Создаём окно
			Stage window = new Stage();
			
			// 2. Создаём элемент
			Label label = new Label();
			label.setText(message);
			
			// 3. Создаём элемент
			Button yesButton = new Button("Yes");
			Button noButton = new Button("No");
			// 3.1 Добавляем к кнопке реакцию при нажатии
			yesButton.setOnAction(e -> {
				answer = true;
				window.close();
			});
			noButton.setOnAction(e -> {
				answer = false;
				window.close();
			});
			
			// 4. Создаём вёрстку элементов в окне
			VBox layout = new VBox(10);
			layout.setAlignment(Pos.CENTER);
			// 4.1 Добавляем элементы в окно
			layout.getChildren().addAll(label, yesButton, noButton);

			// 1.1 Создаём тип окна
			window.initModality(Modality.APPLICATION_MODAL);
			// 1.2 Создаём титульное название окна
			window.setTitle(title);
			// 1.3 Создаём размеры окна
			window.setMinWidth(320);
			window.setMinHeight(240);
			// 1.4 Создаём внутреннее пространство
			Scene scene = new Scene(layout);
			window.setScene(scene);
			// 1.5 Показываем окно
			window.showAndWait();
			
			return answer;
		}
	}
}